...
style.configure("TLabel", background="#F0F4F8", font=("Segoe UI", 10), anchor=tk.W)
style.configure("TEntry", padding=5, relief="flat", font=("Segoe UI", 10))
style.configure("TButton", padding=6, relief="flat", font=("Segoe UI", 10), background="#4CAF50", foreground="white")
style.configure("TButton", padding=6, relief="flat", font=("Segoe UI", 10), background="#4CAF50", foreground="white")
style.configure("Treeview", font=("Segoe UI", 10), background="#FFFFFF", fieldbackground="#FFFFFF", rowheight=30)
...
for i, label in enumerate(labels):
    ttk.Label(frame_form, text=label).grid(row=i//4, column=i%4*2, padx=10, pady=5, sticky=tk.W)
    entry = ttk.Entry(frame_form, width=50)
    entry.grid(row=i//4, column=i%4*2+1, padx=10, pady=5, sticky=tk.W)
    entries[label] = entry
